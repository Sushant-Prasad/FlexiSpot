  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success && response.data.token) {
        ...
        const userRole = response.data.role || "EMPLOYEE"; // Default to EMPLOYEE if role not provided
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 via-white to-blue-200 px-4">
      <div className="bg-white p-8 rounded-2xl shadow-2xl w-full max-w-md border border-blue-100">
        ...
        {msg && (
          <div className={`mb-4 text-center font-semibold rounded py-2 ${success ? 'text-green-700 bg-green-50 border border-green-200' : 'text-red-600 bg-red-50 border border-red-200'}`}>
            {msg}
          </div>
        )}
